...
function isCreditCardNumber(ccn) {
  ...
  const res = arr.reduce((sum, el, index) => {
    let num = index % 2 ? Number(el) * 2 : Number(el);
    num = num > 9 ? num - 9 : num;
    return sum + num;
  }, 0);
  return !(res % 10);
...
function getDigitalRoot(num) {
  ...
  for (let i = 0; i < num.toString().length; i += 1) {
    result1 += Number(num.toString()[i]);
  }
  for (let i = 0; i < result1.toString().length; i += 1) {
    result2 += Number(result1.toString()[i]);
  }
  return result2;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (brackets[str[i]] === last && stack.length >= 1) {
      stack.pop();
...
function getCommonDirectoryPath(pathes) {
  ...
  arr[0].forEach((itemFirst, index) => {
    if (arr.every((itemSec) => itemSec[index] === itemFirst)) {
      result += `${itemFirst}/`;
    }
  });
  return result;
...
function getMatrixProduct(m1, m2) {
  ...
  for (let row = 0; row < m1.length; row += 1) {
    newMatrix[row] = [];
    for (let ver = 0; ver < m2[0].length; ver += 1) {
      let sum = 0;
      for (let i = 0; i < m1[0].length; i += 1) {
        sum += m1[row][i] * m2[i][ver];
      }
      newMatrix[row][ver] = sum;
    }
  }
  return newMatrix;
}
