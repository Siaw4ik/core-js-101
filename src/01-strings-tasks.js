...
function getRectangleString(width, height) {
  ...
  const secondStr = `│${' '.repeat(width - 2)}│\n`;
  const lastStr = `└${'─'.repeat(width - 2)}┘\n`;
  return `${firstStr}${secondStr.repeat(height - 2)}${lastStr}`;
}
...
function encodeToRot13(str) {
  ...
  const arrROT13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

  return str.split('').map((i) => arrROT13[arr.indexOf(i)] || i).join('');
}
